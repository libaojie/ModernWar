...
#region Common functionality
    ...
    //下面这行焦帅添加
    protected Vector3 mLocalPosOnStartDragging;
	...
	{
        //下面这行焦帅添加
        mLocalPosOnStartDragging = mTrans.localPosition;
	...
	public void StopDragging (GameObject go)
	{
		if (mDragging)
		{
            mTrans.localPosition = mLocalPosOnStartDragging;
			mDragging = false;
    ...
    /// 经过测试发现：必须把物体拖到带collider的UI上上释放，surface这个变量才不会是null.
	/// </summary>
	protected virtual void OnDragDropRelease (GameObject surface)
	{
        Debug.Log(" Base::OnDragDropRelease(" + (surface?surface.name:"n/a")+")");
		if (!cloneOnDrag)
		{
